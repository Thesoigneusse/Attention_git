...
def convert_to_serializable(obj):
    ...
    if isinstance(obj, tuple):
        return convert_to_serializable(list(obj))
    if isinstance(obj, dict):
        return {k: convert_to_serializable(v) for k, v in obj.items()}
    if isinstance(obj, list):
        return [convert_to_serializable(i) for i in obj]
    if not isinstance(obj, int) and not isinstance(obj, str) and not isinstance(obj, float):
    ...
    for idx, source_sentence in enumerate(align_data):
        ...
        if idx >= 0: # and idx < 2127:
            ...
            if not skip:
                ...
                if True: #idx >= 3:
                    sys_cur_corefs = find_coref_matches(source_sentence.raw_parcorfull_sentence, cur_seq, a)
...
def main(args):
    ...
    if _VERBOSE:
        for source_sentence, target_sentence in zip(aligned_src, aligned_tgt):
